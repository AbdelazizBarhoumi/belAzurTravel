...
class SendAdminDigest extends Command
{
    ...
    public function handle(): int
    {
        ...
        $audits = BookingAudit::query()->where('created_at', '>=', $since)->get()->groupBy('action');
    ...
    {
        return [
            'title' => 'Booking #'.$booking->id.' — '.($booking->client['name'] ?? $booking->client['email'] ?? 'Client').' ('.$booking->status.')',
    ...
    private function auditItems(Collection $audits, array $actions): array
    {
        return $audits
            ...
            ->map(fn (BookingAudit $audit) => [
                'title' => 'Booking #'.$audit->booking_id.' — '.$audit->action,
