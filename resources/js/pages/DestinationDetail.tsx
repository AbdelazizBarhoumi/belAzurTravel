...
const DESTINATIONS: DestinationItem[] = [
    {
        ...
        highlights: [
            { fr: 'Vues sur la caldeira au coucher du soleil', ar: 'إطلالات الغروب على الكالديرا', en: 'Sunset caldera views' },
            { fr: 'Villages blanchis à la chaux sur la falaise', ar: 'قرى بيضاء على المنحدرات', en: 'Whitewashed cliff villages' },
            { fr: 'Dégustations de vin', ar: 'تجارب تذوق النبيذ', en: 'Wine tasting experiences' },
        ],
    ...
    },
    {
        ...
        highlights: [
            { fr: 'Promenades dans les rizières', ar: 'نزهات مدرجات الأرز', en: 'Rice terrace walks' },
            { fr: 'Rituels des temples balinais', ar: 'طقوس معابد بالي', en: 'Balinese temple rituals' },
            { fr: 'Retraites spa et bien-être', ar: 'منتجعات سبا وعافية', en: 'Spa and wellness retreats' },
        ],
    ...
    },
    {
        ...
        highlights: [
            { fr: 'Louvre et promenades sur la Seine', ar: 'اللوفر ونزهات على السين', en: 'Louvre and Seine walks' },
            { fr: 'Scène gastronomique raffinée', ar: 'مشهد طعام راقٍ', en: 'Gourmet dining scene' },
            { fr: 'Vues sur les monuments emblématiques', ar: 'إطلالات على المعالم الشهيرة', en: 'Iconic landmark views' },
        ],
    ...
    },
    {
        ...
        highlights: [
            { fr: 'Safaris dans le désert', ar: 'رحلات سفاري في الصحراء', en: 'Desert safaris' },
            { fr: 'Terrasses panoramiques sur l’horizon', ar: 'منصات مشاهدة الأفق', en: 'Skyline viewing decks' },
            { fr: 'Resorts luxueux en bord de mer', ar: 'منتجعات شاطئية فاخرة', en: 'Luxury beachfront resorts' },
        ],
...
const HOTELS: HotelItem[] = [
    { slug: 'sunset-paradise-resort', name: { fr: 'Sunset Paradise Resort', ar: 'منتجع صن ست بارادايس', en: 'Sunset Paradise Resort' }, image: 'https://images.unsplash.com/photo-1571896349842-33c89424de2d?w=800&h=500&fit=crop', price: 320, destinationSlug: 'santorini' },
    { slug: 'cliffside-boutique-santorini', name: { fr: 'Cliffside Boutique Santorini', ar: 'كلفسايد بوتيك سانتوريني', en: 'Cliffside Boutique Santorini' }, image: 'https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?w=800&h=500&fit=crop', price: 410, destinationSlug: 'santorini' },
    { slug: 'paris-grand-hotel', name: { fr: 'Le Grand Parisien', ar: 'لو غران باريزيان', en: 'Le Grand Parisien' }, image: 'https://images.unsplash.com/photo-1551882547-ff40c63fe5fa?w=800&h=500&fit=crop', price: 450, destinationSlug: 'paris' },
    { slug: 'dubai-skyline-stay', name: { fr: 'Marina Bay Suites', ar: 'مارينا باي سويتس', en: 'Marina Bay Suites' }, image: 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=800&h=500&fit=crop', price: 280, destinationSlug: 'dubai' },
    { slug: 'bali-garden-villas', name: { fr: 'Ubud Garden Villas', ar: 'أوبود غاردن فيلات', en: 'Ubud Garden Villas' }, image: 'https://images.unsplash.com/photo-1537996194471-e657df975ab4?w=800&h=500&fit=crop', price: 220, destinationSlug: 'bali' },
];
...
export default function DestinationDetail() {
    ...
    const relatedTours = TOURS.filter((tour) =>
        localize(tour.location, lang).toLowerCase().includes(localize(destination.country, lang).toLowerCase())
        || localize(tour.location, 'en').toLowerCase().includes(localize(destination.country, 'en').toLowerCase()),
    );
        ...
        <div className="min-h-screen bg-background">
            ...
            <main className="pb-16 pt-24">
                <div className="container mx-auto px-4">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12 grid gap-8 lg:grid-cols-[2fr_1fr]">
                            ...
                            <section className="mt-8 mb-12">
                                ...
                                <div className="grid max-w-3xl gap-3 sm:grid-cols-2">
                                    {destination.highlights.map((highlight) => (
                                        <div key={highlight.en} className="flex items-center gap-3 rounded-xl border border-border bg-card p-3">
                                            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/10">
                                                <Check className="h-4 w-4 text-primary" />
                                            </div>
                                            <span className="text-sm text-foreground">{localize(highlight, lang)}</span>
                                        </div>
                                    ))}
                                </div>
                            </section>
                        ...
                        <aside>
                            <StickyBookingCard
                                ...
                                onBook={() => window.open(`/design-trip?destination=${destination.slug}`, '_self')}
                            />
                        </aside>
                    </motion.div>
                                ...
                                {relatedHotels.map((hotel) => (
                                    <Link key={hotel.slug} to={`/hotels/${hotel.slug}`} className="group overflow-hidden rounded-2xl bg-card card-elevated">
                                        <div className="h-40 overflow-hidden">
                                            <img src={hotel.image} alt={localize(hotel.name, lang)} className="h-full w-full object-cover transition-transform group-hover:scale-105" />
                                        </div>
                                        <div className="p-4">
                                            <h3 className="font-serif font-bold text-foreground">{localize(hotel.name, lang)}</h3>
