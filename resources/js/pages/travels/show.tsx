...
import { useTravelDetailsBySlug } from '@/hooks/usePublicData';
...
export default function TravelDetail() {
    ...
    const { data: travel, isLoading } = useTravelDetailsBySlug(slug);
    ...
    const otherImages =
        Array.isArray(travel.gallery) && travel.gallery.length > 0
            ? travel.gallery
            : Array.isArray(travel.images) && travel.images.length > 0
              ? travel.images
              : [];
    ...
    const galleryImages = travel.image
        ? [travel.image, ...otherImages.filter((i) => i !== travel.image)]
        : otherImages.length
          ? otherImages
          : [];
    ...
    return (
        <PageShell
            breadcrumbs={[
                ...
                {
                    label:
                        typeof travel.name === 'object'
                            ? localize(travel.name, lang)
                            : travel.name,
                    active: true,
                },
            ]}
                ...
                <div className="space-y-8">
                    ...
                    <div className="lg:hidden">
                        <StickyBookingCard
                            ...
                            priceSuffix={t('travels.person')}
                            title={
                                typeof travel.name === 'object'
                                    ? localize(travel.name, lang)
                                    : travel.name
                            }
                            location={localizeLocation(travel.location, lang)}
                            description={
                                typeof travel.description === 'object'
                                    ? localize(travel.description, lang)
                                    : travel.description
                            }
                            entityType="travel"
                            itemSlug={travel.slug}
                            duration={`${travel.durationDays} ${t('common.days')} / ${travel.durationNights} ${t('common.nights')}`}
                            maxGroup={travel.maxGroup}
                            rating={travel.rating}
                            primaryButtonLabel={t('travels.bookTravel')}
                        />
                    </div>
