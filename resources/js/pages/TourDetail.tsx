...
import { useTourDetailsBySlug } from '@/hooks/useCatalog';
            ...
            <section className="grid max-w-4xl gap-6 md:grid-cols-2">
                <div className="rounded-2xl border border-border bg-card p-6">
                    ...
                    <ul className="space-y-2">
                        {(includes || []).map((i) => (
                            <li
                                key={typeof i === 'string' ? i : i.en}
                                className="flex items-start gap-2 text-sm text-foreground"
                            >
                                <CheckIcon className="mt-0.5 h-4 w-4 shrink-0 text-primary" />
                                {typeof i === 'string' ? i : localize(i, lang)}
                            </li>
                        ))}
                    </ul>
                </div>
...
export default function TourDetail() {
    ...
    const { data: tour } = useTourDetailsBySlug(slug);
                            ...
                            <div className="lg:hidden">
                                <StickyBookingCard
                                    ...
                                    favoriteItem={{
                                        ...
                                        image: tour.images?.[0] ?? '',
                                        price: tour.pricePerPerson,
                                        location: Array.isArray(tour.location)
                                            ? tour.location
                                                  .map((item) =>
                                                      typeof item === 'string'
                                                          ? item
