...
import type { AdminTravel } from '@/hooks/useAdminStore';
import type { Lang } from '@/i18n/translations';
import { localizeKnown, tourLabels } from '@/lib/adminI18n';

type TourFormValues = AdminTravel &
...
const simpleLocalizedSchema: JsonFieldDef[] = [
    { key: 'name', labelKey: 'admin.travelForm.itemName', translatable: true },
];
...
const AdminTravels = () => {
    ...
    const isCodeEnabled =
        siteSettings?.config?.navigation?.enabled_dropdowns?.includes('travels');
    const [open, setOpen] = useState(false);
    const [catManagerOpen, setCatManagerOpen] = useState(false);

    const { data: travels = [] } = useQuery({
        queryKey: ['admin', 'travels'],
        queryFn: () => listAdminEntities<AdminTravel>('travels'),
    });

    const { data: categoryTypes = [] } = useCategoryTypes('travels');
    ...
    const saveMutation = useMutation({
        mutationFn: (item: AdminTravel) => saveAdminEntity('travels', item),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin'] }),
    });

    const deleteMutation = useMutation({
        mutationFn: (id: string) => deleteAdminEntity('travels', id),
        onSuccess: () => {
    ...
    const [editing, setEditing] = useState<AdminTravel | null>(null);
    const [pendingDelete, setPendingDelete] = useState<AdminTravel | null>(null);
    ...
    const handleSave = (values: TourFormValues) => {
        ...
        saveMutation.mutate(payload as unknown as AdminTravel, {
            onSuccess: () => {
                toast.success(
                    editing ? t('admin.travelUpdated') : t('admin.travelAdded'),
                );
                setOpen(false);
                setEditing(null);
                setErrors({});
    ...
    const tourSections: SectionDef[] = [
        {
            title: t('admin.travelForm.coreDetails'),
            column: 'main',
            description: t('admin.travelForm.coreDetailsHint'),
        ...
        },
        {
            title: 'Filters',
            column: 'main',
            description: 'Travel filter options',
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {[
                                ...
                                { key: 'afrique_nord', label: 'Afrique du Nord' },
                            ].map((item) => (
        ...
        {
            title: t('admin.travelForm.itinerary'),
            column: 'main',
            description: t('admin.travelForm.itineraryHint'),
            render: ({ values, setField, activeLang }) => (
                <JsonListEditor
                    title={t('admin.travelForm.itinerary')}
                    items={
                    ...
                    addButtonLabel={t('admin.travelForm.addDay')}
                    itemLabel={(item, index) =>
                        (item.title as Record<string, string> | undefined)?.[
                            activeLang
                        ] || `${t('admin.travelForm.day')} ${index + 1}`
                    }
                />
            ),
        },
        {
            title: t('admin.travelForm.inclusions'),
            column: 'main',
            description: t('admin.travelForm.inclusionsHint'),
            render: ({ values, setField, activeLang }) => (
                <div className="space-y-6">
                    <JsonListEditor
                        title={t('admin.travelForm.includes')}
                        items={
                        ...
                        activeLang={activeLang}
                        addButtonLabel={t('admin.travelForm.addInclusion')}
                        itemLabel={(item, index) =>
                            (item.name as Record<string, string> | undefined)?.[
                                activeLang
                            ] || `${t('admin.voyageForm.item')} ${index + 1}`
                        }
                    />
                    <JsonListEditor
                        title={t('admin.travelForm.excludes')}
                        items={
                        ...
                        schema={simpleLocalizedSchema}
                        activeLang={activeLang}
                        addButtonLabel={t('admin.travelForm.addExclusion')}
                        itemLabel={(item, index) =>
                            (item.name as Record<string, string> | undefined)?.[
                                activeLang
                            ] || `${t('admin.travelForm.item')} ${index + 1}`
                        }
                    />
                </div>
            ),
    ...
    return (
        <AdminLayout
            title={t('admin.travels')}
            subtitle={t('admin.travelsSubtitle')}
            actions={
        ...
        >
            <CategoryTypeManager
                ...
                onClose={() => {
                    ...
                    queryClient.invalidateQueries({
                        queryKey: ['admin', 'category-types', 'travels'],
                    });
                }}
            />
            ...
            <EntityFormDialog<Record<string, unknown>>
                ...
                title={
                    editing
                        ? `${t('actions.edit')} ${t('admin.travels')}`
                        : `${t('actions.add')} ${t('admin.travels')}`
                }
