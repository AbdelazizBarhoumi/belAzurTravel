...
export function OrganizedSection({ config }: Props) {
    ...
    const displayItems = style === 'grid' ? groupTravels.slice(0, 6) : groupTravels.slice(0, 3);
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {displayItems.map((travel, i) => (
                        <Link key={travel.slug} to={`/travels/${travel.slug}`} className="group">
                            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                                <div className="relative h-48 overflow-hidden">
                                    <img src={travel.image} alt={localizeText(travel.name, lang)} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
                                    <div className="absolute right-3 top-3 rounded-full bg-card/95 px-3 py-1 text-xs font-bold shadow-md backdrop-blur">{travel.price} DT</div>
                                </div>
                                <div className="p-4">
                                    <div className="flex items-center gap-1 text-xs text-muted-foreground">
                                        <MapPin className="h-3 w-3" /> {localizeText(travel.location, lang)}
                                    </div>
                                    <h3 className="mt-1 font-serif text-lg font-bold">{localizeText(travel.name, lang)}</h3>
                                    <div className="mt-2 flex items-center gap-3 text-xs text-muted-foreground">
                                        <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {localizeText(travel.duration, lang)}</span>
