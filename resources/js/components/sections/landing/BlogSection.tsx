...
export function BlogSection({ config }: Props) {
    ...
    const title = config.hideTitle
        ? ''
        : (config.title?.[lang] ?? config.title?.en ?? t('home.latestBlog'));
    const subtitle = config.hideTitle
        ? ''
        : (config.subtitle?.[lang] ?? config.subtitle?.en ?? t('home.latestBlogDesc'));
                            ...
                            >
                                ...
                                >
                                    <div className="relative h-48 overflow-hidden">
                                        <img
                                            src={post.image}
                                            alt={localizeText(post.title, lang)}
                                        ...
                                        <div className="absolute left-3 top-3 rounded-full bg-primary/90 px-3 py-1 text-xs font-bold text-primary-foreground">
                                            {localizeText(post.category, lang)}
                                        </div>
                                    </div>
                                    <div className="p-5">
                                        ...
                                        <h3 className="mt-1 line-clamp-2 font-serif text-lg font-bold">
                                            {localizeText(post.title, lang)}
                                        </h3>
                                        <p className="mt-2 line-clamp-2 text-sm text-muted-foreground">
                                            {localizeText(post.excerpt, lang)}
                                        </p>
                                    </div>
                                </motion.div>
                            </Link>
