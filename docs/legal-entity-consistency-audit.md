...
- **Mapping Mismatch:** The API controller performs manual transformation from database snake*case keys (e.g., `company_name`) to camelCase (e.g., `companyName`). While consistent within the API response, any changes to the DB schema require updating both the migration, the model, \_and* the manual mapping logic in `SiteSettingsController`.
